...
//      (() => userService.getCurrentContext)
//        .expects()
//        .throwing(new SharedExceptions.UnauthorizedException)
//
//      val model = ModelProperty(ChatModel("", Seq.empty, "", 0))
//      recoverToSucceededIf[SharedExceptions.UnauthorizedException](
//        Future(new GamePresenter(model, chatRpc, userService, notificationsCenter))
//      )
//    }
//
...
//      (() => chatRpc.connectedClientsCount())
//        .expects()
//        .once()
//        .returning(Future.successful(connectionsCount))
//
//      val userService = mock[UserContextService]
...
//
//      presenter.handleState(GameState)
//
//      Future {
...
//      (notificationsCenter.onConnectionsCountChange _)
//        .expects(*)
//        .once()
//        .returning(new DummyRegistration)
//      val chatRpc = mock[ChatRPC]
//      val userService = mock[UserContextService]
//      (() => userService.getCurrentContext)
//        .expects()
//        .returning(UserContext(UserToken("t1"), "u1", Set.empty))
//
//      val model = ModelProperty(ChatModel(Seq.empty, "", 0))
//      val presenter = new ChatPresenter(model, chatRpc, userService, notificationsCenter)
